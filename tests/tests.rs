...
fn should_update_test() {
    ...
    assert!(bst.insert_or_update_if(key, value, &should_update).is_ok_and(|x| x));
    value += 1;
    assert!(bst.insert_or_update_if(key, value, &should_update).is_ok_and(|x| x));
    value -= 1;
    assert!(!bst.insert_or_update_if(key, value, &should_update).is_ok_and(|x| x));
...
fn bench_insert_or_update_if(){
    ...
    for _ in 0..total{
        if bst.insert_or_update(key, value).is_ok_and(|x| x) {true_count += 1};
        value += 1;
    }
                ...
                threads.push(tokio::spawn(async move{
                    ...
                    USER_LIST.read().map(|read_lock| {
                        for i in start_index..(start_index+TOTAL_PER_THREAD) {
                            ...
                            if GLOBAL_BST.insert_or_update_if(user.0, user.1, &should_update).is_ok_and(|x| x){
