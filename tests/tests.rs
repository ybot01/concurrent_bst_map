...
use concurrent_bst::{ConcurrentBSTMap, ALWAYS_UPDATE};
...
fn length_test(){
    ...
    get_vec_of_key_values::<(u64,u64)>(expected).iter()
        .for_each(|x| _ = bst.insert_or_update(x.0, x.1, &ALWAYS_UPDATE, None));
...
fn remove_test(){
    ...
    let bst = ConcurrentBSTMap::<u64, u64>::new();
    to_insert.iter().for_each(|x| _ = bst.insert_or_update(x.0, x.1, &ALWAYS_UPDATE, None));
...
fn should_update_test() {
    ...
    assert!(bst.insert_or_update(key, value, &should_update, None).is_ok_and(|x| x));
    value += 1;
    assert!(bst.insert_or_update(key, value, &should_update, None).is_ok_and(|x| x));
    value -= 1;
    assert!(!bst.insert_or_update(key, value, &should_update, None).is_ok_and(|x| x));
...
fn insert_and_get_test() {
    let bst = ConcurrentBSTMap::<u64, u64>::new();
    _ = bst.insert_or_update(0, 1, &ALWAYS_UPDATE, None);
...
fn bench_insert_or_update_if(){
    ...
    for _ in 0..total{
        if bst.insert_or_update(key, value, &should_update, None).is_ok_and(|x| x) {true_count += 1};
        value += 1;
    }
                ...
                threads.push(tokio::spawn(async move{
                    ...
                    USER_LIST.read().map(|read_lock| {
                        for i in start_index..(start_index+TOTAL_PER_THREAD) {
                            ...
                            if GLOBAL_BST.insert_or_update(user.0, user.1, &should_update, None).is_ok_and(|x| x){
